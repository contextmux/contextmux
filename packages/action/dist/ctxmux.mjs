...
function provenanceHeader(filePath, hash2) {
  const c2 = commentSyntax(filePath);
  if (!c2) return null;
  const text = `Generated by contextmux from .ctxmux/ \u2014 do not edit by hand. ctxmux:hash=${hash2}`;
...
function renderBlock(file, existing, provenance) {
  ...
  const hash2 = hashContent(file.content);
  const note = provenance ? ` ctxmux:hash=${hash2}` : "";
...
async function buildIndex(root, opts = {}) {
  ...
  for (const rel of paths) {
    ...
    const hash2 = createHash2("sha1").update(content).digest("hex").slice(0, 16);
    const cached = cache.entries[rel];
    const symbols = cached && cached.hash === hash2 ? cached.symbols : extractSymbols(content, ext);
    nextCache.entries[rel] = { hash: hash2, symbols, bytes: stat.size };
    files.push({ path: rel, ext, bytes: stat.size, symbols, hash: hash2 });
  }
...
function digest(text) {
  return createHash3("sha256").update(text).digest("base64url").slice(0, 22);
}
...
function traceIdFor(runId) {
  return createHash4("sha256").update(`contextmux:trace:${runId}`).digest("hex").slice(0, 32);
}
function spanIdFor(runId, seq) {
  return createHash4("sha256").update(`contextmux:span:${runId}:${seq}`).digest("hex").slice(0, 16);
}
...
function existedFlag(cwd, toolName, files) {
  ...
  try {
    return { existed: files.every((f) => existsSync(path10.resolve(cwd, f))) };
  } catch {
    return {};
  }
}
...
async function askOnce(spec, input) {
  if (!spec.askOnly) {
  ...
  return new Promise((resolve18) => {
    ...
    child.on("close", (code) => {
      ...
      const text = typeof fromJson?.["result"] === "string" ? fromJson["result"] : textFromStream(out) || out;
      finish({ ok: true, text });
    });
  });
}
...
async function writeFileAtomic2(file, content) {
  ...
  const tmp = path11.join(dir, `.${path11.basename(file)}.ctxmux-${process.pid}-${sequence2++}.tmp`);
  try {
...
var FileStore = class {
  ...
  dir;
  runPath(runId) {
    return path12.join(this.dir, "runs", `${encodeURIComponent(runId)}.json`);
  }
  /** Legacy single-file record, still read so an existing installation is not re-applied. */
  legacyAppliedPath() {
    return path12.join(this.dir, "applied.json");
  }
  /**
  ...
  appliedMarkerPath(key) {
    const digest2 = createHash5("sha256").update(key).digest("hex").slice(0, 32);
    return path12.join(this.dir, "applied", `${digest2}.key`);
  }
  leasePath(runId) {
    return path12.join(this.dir, "leases", `${encodeURIComponent(runId)}.lease`);
  }
  async load(runId) {
    try {
  ...
  async list() {
    try {
      const files = await fs10.readdir(path12.join(this.dir, "runs"));
   ...
   */
  async forgetApplied(runId) {
    const dir = path12.join(this.dir, "applied");
    let names;
    try {
  ...
  async applyOnce(key, fn) {
    ...
    await fs10.mkdir(path12.dirname(marker), { recursive: true });
    try {
  ...
  async acquireLease(runId, ttlMs) {
    ...
    await fs10.mkdir(path12.dirname(p), { recursive: true });
...
// packages/council/src/criteria.ts
var RUBRIC_VERSION = "1.0.0";
var CRITERIA = [
  {
    id: "actionable",
    question: "Does this say what to do, rather than only describing how things are?",
    rationale: 'An agent cannot follow a description. "The project uses a compiler" tells it nothing; "compile before committing" does. This is the most common way a rule ends up inert.',
    minDepth: "single"
  },
  {
    id: "checkable",
    question: "Could a reviewer tell from a diff whether this was followed?",
    rationale: 'A rule nobody can check is a preference, and it will be ignored without anyone noticing. "Keep functions small" cannot be checked; "no function longer than the screen it is read on" can be argued about, which is already better.',
    minDepth: "single"
  },
  {
    id: "earns-its-place",
    question: "Does this tell a competent agent something it would not already do by default?",
    rationale: 'Every rule is read on every task and costs context that something else could have used. "Write clean code", "add tests", "handle errors" are already defaults. A rule that restates one is worse than absent: it dilutes the rules that carry real information.',
    minDepth: "single"
  },
  {
    id: "grounded",
    question: "Does this match how the repository actually works?",
    rationale: "Rules outlive the code they were written about. A rule naming a framework that was replaced, or a directory that moved, sends an agent somewhere that no longer exists.",
    minDepth: "panel"
  },
  {
    id: "consistent",
...
function criteriaFor(depth) {
  if (depth === "static") return [];
  if (depth === "single") return CRITERIA.filter((c2) => c2.minDepth === "single");
  return CRITERIA;
}
...
function buildPrompt(nodes, criteria, context, known = []) {
  ...
  for (const c2 of criteria) {
    lines.push(`- ${c2.id}: ${c2.question}`);
    lines.push(`  ${c2.rationale}`);
  }
  lines.push(
    "",
    "The rules:",
...
function parseFindings(answer, known) {
...
async function advise(root) {
  ...
  return {
    findings: inspect2(loaded.model, { files: tracked ?? [], targets: loaded.config.targets }),
...
async function initCommand(args) {
  ...
  if (foundExisting && imported) {
    for (const file of imported.files) {
      await writeFileAtomic(path13.join(root, file.path), file.content);
      written.push(file.path);
    }
    heading("Imported");
...
async function doctorCommand(args) {
  ...
  if (ctx && ctx.model.mcp.length > 0) {
    ...
    for (const server of ctx.model.mcp) {
      ...
      if (looksLocal && !await exists2(path14.resolve(root, server.command))) {
        checks.push({
  ...
  const workflowDir = path14.join(root, ".github", "workflows");
  const workflowNames = await fs12.readdir(workflowDir).catch(() => []);
  for (const name of workflowNames.filter((n) => /\.ya?ml$/.test(n))) {
    const body = await fs12.readFile(path14.join(workflowDir, name), "utf8").catch(() => "");
  ...
  if (ctx) {
    for (const target of ctx.config.targets) {
      ...
      for (const f of result.files) {
        if (!await exists2(path14.resolve(root, f.path))) missing.push(f.path);
      }
      checks.push({
...
// packages/notifier-teams/src/index.ts
var COLOUR = {
  info: "default",
  warn: "warning",
  error: "attention"
};
var TeamsNotifier = class {
  constructor(opts) {
    this.opts = opts;
    this.fetchImpl = opts.fetchImpl ?? fetch;
  }
  ...
  async send(event) {
    const card = {
      ...
      body: [
        ...
        ...event.runId ? [{ type: "TextBlock", text: `run: ${event.runId}`, isSubtle: true, spacing: "Small" }] : []
      ]
    };
    const res = await this.fetchImpl(this.opts.webhookUrl, {
      method: "POST",
...
var PREFIX = "bridged-from:";
function bridgeLabel(tracker, id) {
  return `${PREFIX}${tracker}:${id}`;
}
function parseBridgeLabel(labels) {
...
var REPORTABLE_STATES = ["completed", "escalated", "failed", "rejected", "in_review"];
var REPORT_VERB = {
  completed: "completed",
...
function bridgeOutcomeMessage(task, run3) {
  if (!REPORTABLE_STATES.includes(run3.state)) return null;
  const bridge = parseBridgeLabel(task.labels);
  if (!bridge) return null;
  const prUrl = run3.result?.location?.prUrl;
  const issueUrl = task.origin.url ?? `issue ${task.origin.id}`;
  const body = [
    `The bridged GitHub issue (${issueUrl}) is ${REPORT_VERB[run3.state] ?? run3.state}.`,
    ...prUrl ? [prUrl] : []
  ].join("\n");
  return { bridge, body };
}
async function reportBridgeOutcome(task, run3, resolveOptions) {
  const outcome = bridgeOutcomeMessage(task, run3);
  if (!outcome) return;
  try {
    ...
    warn(
      `Could not report back to ${outcome.bridge.id} on the ${outcome.bridge.tracker} tracker: ${err.message}`
    );
  }
}
async function notifyOutcome(notifiers, task, run3) {
  if (notifiers.length === 0) return;
  if (run3.state !== "escalated" && run3.state !== "failed") return;
  const level = run3.state === "escalated" ? "warn" : "error";
  const title = `${task.id} ${run3.state === "escalated" ? "needs a human" : "failed"}`;
  const body = [run3.terminalReason?.split("\n")[0], task.origin.url].filter(Boolean).join("\n");
  await Promise.all(
    notifiers.map(
      (notifier) => notifier.send({ level, title, body, runId: run3.id }).catch((err) => {
        warn(`Could not notify via ${notifier.id}: ${err.message}`);
      })
    )
  );
}
...
async function runCommand(args) {
  ...
  try {
    ...
    if (!dryRun) {
      await reportBridgeOutcome(task, run3, resolveOptions);
      await notifyOutcome(resolveNotifiers(), task, run3);
    }
...
async function statusCommand(args) {
  ...
  const stateOrder = ["completed", "in_review", "escalated", "rejected", "failed"];
  const counted = /* @__PURE__ */ new Map();
  for (const run3 of runs) counted.set(run3.state, (counted.get(run3.state) ?? 0) + 1);
  const breakdown = [
    ...stateOrder.filter((s) => counted.has(s)),
    ...[...counted.keys()].filter((s) => !stateOrder.includes(s))
  ].map((s) => `${counted.get(s)} ${badge[s] ?? s}`).join(c.dim(" \xB7 "));
...
async function planCommand(args) {
  ...
  if (existing) {
    ...
    const context = await loadContext({ root }).then((ctx) => ctx.model, () => void 0);
    const index = await buildIndex(root).catch(() => void 0);
    const audience = agentName && DELEGATED_AGENTS.has(agentName) ? "delegated" : "driven";
    const body2 = renderPrompt({
      task: existing,
      ...context ? { context } : {},
      ...index ? { index } : {},
    ...
    if (body2.length > MAX_ISSUE_BODY) {
      error(
        `The rendered artefact for ${existing.id} is ${body2.length.toLocaleString()} characters, and a GitHub issue body holds ${MAX_ISSUE_BODY.toLocaleString()}.`
      );
    ...
    await tracker.comment(existing.id, `Bridged to GitHub for review: ${bridged.origin.url ?? `#${bridged.id}`}`).catch((err) => {
      warn(`Bridged, but could not link back from ${existing.id} on ${tracker.id}: ${err.message}`);
    });

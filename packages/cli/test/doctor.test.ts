...
import { workflowFiles } from '../src/workflows.js'
import {
  argv,
  makeRepo,
  MINIMAL_CONTEXT,
...
describe('scaffolded workflows', () => {
  ...
  it('flags a run/review workflow that predates a feature the Action gained since', async () => {
    // A run.yml scaffolded before share-state existed at all — the realistic case this check
    // exists for, per its own comment: written once and never regenerated, so one scaffolded a
    // while ago does not gain inputs the Action added since. Built from scratch rather than by
    // stripping the one field out of a current template, because the current template's own
    // permissions comment also happens to mention the word "share-state" in passing — deleting
    // only the config line leaves that behind and the naive substring check never notices.
    await writeAll(root, {
      '.github/workflows/ctxmux-run.yml': [
        '# contextmux run',
        '# Generated by contextmux (ctxmux:workflow). Yours to edit — nothing regenerates it.',
    ...
    expect(text).toContain('predates share-state')
  })

  it('does not flag the check workflow for missing share-state — it never had it to begin with', async () => {
    const files = workflowFiles({ profile: fakeProfile, tracker: 'file', hasRemote: true, agent: 'claude' })
    await writeAll(root, Object.fromEntries(files.map((f) => [f.path, f.content])))

    const { text } = await runCli(doctorCommand, argv(root, 'doctor'))

    // Silently excluded rather than flagged, since it never had share-state to predate — the
    // absence of a "workflow: ctxmux-check.yml" line at all is the correct outcome here, not
    // an oversight.
    expect(text).not.toContain('ctxmux-check.yml')
    expect(text).toContain('ctxmux-run.yml')
    expect(text).toContain('ctxmux-review.yml')
  })
})

...
/**
 * Which agents this repository actually uses, judged by paths on disk — not import
 * provenance. Presence of `.cursor/`, `.claude/`, Copilot instruction paths, or
 * `AGENTS.md` is evidence of use. Empty means nobody has claimed a target yet.
 */
export async function detectTargets(root: string): Promise<Target[]> {
  ...
  if (await exists('.claude') || await exists('CLAUDE.md')) found.add('claude')
  if (
    (await exists('.github/copilot-instructions.md')) ||
    (await exists('.github/instructions'))
  ) {
    found.add('copilot')
  }
  if (await exists('.cursor')) found.add('cursor')
  if (await exists('AGENTS.md')) found.add('codex')
  return [...found]
}

/** @deprecated Prefer detectTargets(root). Kept for call sites that still pass provenance. */
export function detectTargetsFromProvenance(provenance: ImportResult['provenance']): Target[] {
  const found = new Set<Target>()
